...
class Timer():
    ...
    def __call__(self, func, *args):
        """Вызов класса как обычного декоратора"""
        def wrapper():
            """Стандартная обертка для декоратора. Далее все тоже что и в __exit__."""
            print(f"Начало замера функции {self.func}. Количество запусков {self.num_runs}.")
            for i in range(self.num_runs):
                print(f"---|{i + 1}|--- запуск функции")
                # Отметка начала выполнения
                start = time.time()
            ...
            # Окончив выполнять функцию num_runs раз считаем среднее время выполнения функции
            self.sum_time /= self.num_runs
            print(f"Среднее время работы функции {round(self.sum_time, 4)} секунд.")
        return wrapper

# Вызов класса в качестве декоратора(Для проверки уберите комментирование декоратора)
# @Timer(20)
...
if __name__ == "__main__":
    ...
    # Для проверки вызова класса как @декоратора(Для проверки уберите комментирование вызова функции fibo())
    # fibo()